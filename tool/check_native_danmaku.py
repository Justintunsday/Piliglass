...
Future<void> main() async {
  ...
  expect(rows(await call('load', {'profile': 'simple'})).length == 3, 'both player profiles inherit legacy rules once');
  ...
  final simple = await call('load', {'profile': 'simple'});
  expect(simple['weight'] == 0 && simple['area'] == 0.5 && simple['opacity'] == 1.0, 'full settings cannot change simple settings');
  expect(rows(simple).any((e) => e['filter'] == 'legacy'), 'editing full rules cannot edit simple rules');
  result = await call('save', {...settings, 'profile': 'simple', 'weight': 2, 'enabled': false, 'opacity': 0.4, 'strokeWidth': 2.5});
  expect(result['enabled'] == false && result['weight'] == 2 && result['strokeWidth'] == 0.0, 'simple profile saves independently with zero outline');
  result = await call('load');
  expect(result['enabled'] == true && result['weight'] == 11 && result['opacity'] == 0.0, 'simple settings and toggle cannot change full settings');
  await call('add', {'profile': 'simple', 'type': 0, 'filter': 'only-simple'});
  expect(!rows(await call('load')).any((e) => e['filter'] == 'only-simple'), 'simple local rules never leak into full profile');
  final reopened = await NativeDanmakuSettings().handle({'action': 'load', 'profile': 'simple'});
  expect(reopened['enabled'] == false && reopened['weight'] == 2 && rows(reopened).any((e) => e['filter'] == 'only-simple'), 'simple settings and rules survive a new service session');
  GStorage.setting.values[SettingBoxKey.danmakuStrokeWidth] = 2.5;
  expect((await call('load'))['strokeWidth'] == 0.0, 'legacy full outline is ignored after control removal');
  expect((await call('save', {...settings, 'strokeWidth': 2.5}))['strokeWidth'] == 0.0 && Pref.danmakuStrokeWidth == 0.0, 'full saves cannot reintroduce outlines');
  expect((await call('save', {...settings, 'profile': 'unknown'}))['state'] == 'error', 'unknown profile cannot overwrite either player');
  Accounts.main..isLogin = true..mid = 42;
  expect(rows(await call('load')).isEmpty, 'guest rules do not leak into account');
  expect(rows(await call('load', {'profile': 'simple'})).isEmpty, 'simple guest rules also respect account isolation');
  ...
  expect(rows(await call('load', {'profile': 'simple'})).isEmpty, 'full cloud sync does not replace simple rule cache');
