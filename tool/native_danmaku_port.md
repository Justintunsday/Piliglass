...
- 完整播放器（全屏）与简易播放器（播放页内嵌）分别保存弹幕开关、显示参数、
  类型筛选、屏蔽等级及本地规则缓存。切换模式前保存旧模式，再加载新模式，
  过期的异步响应不会覆盖当前模式。
- 升级时两套配置均继承原有设置，之后互不覆盖。完整播放器沿用原有 Hive
  设置键，简易播放器使用独立设置；描边固定为 0.0，设置入口已移除，旧值不再生效。
- “关键词 / 正则 / 用户”中可新增、编辑、删除；用户规则输入 UID，
  复用原有 CRC32 转换，展示与弹幕 midHash 相同的哈希。
- 本地规则未登录亦可使用。登录后可选择保存到账号，或同步账号规则。
  同步为从服务端刷新账号规则，不会上传或删除本地规则。
- 两个播放器的规则缓存独立；Bilibili 账号的云端规则仍属于同一个账号，
  在另一个播放器手动同步时也会读取服务端的最新规则。
...
失败保留、云端部分成功、账号切换、并发写入、播放器配置隔离及取消描边检查，
并执行 Dart analyze。
