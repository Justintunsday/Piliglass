...
final class VideoSegmentProvider: HLSSegmentProvider, @unchecked Sendable {
    ...
    // PiliGlass: over-declared fallbacks. Under-declaring fires
    // CoreMediaErrorDomain -12318 "Segment exceeds specified bandwidth" on
    // every segment once real content outruns the estimate, which happened
    // with the upstream 25 Mbps floor on high-bitrate Bilibili streams
    // (source bitrate is frequently 0 for Bilibili m4s). AVPlayer serves a
    // single local rendition here, so a generous declaration has no
    // selection side effect; 100 Mbps covers 8K/DoVi peaks.
    var masterAverageBandwidth: Int? {
        sourceBitrate > 0 ? Int(sourceBitrate) : 100_000_000
    }

    /// 2x average as peak estimate (4K HDR action bursts to ~2x); generous
    /// floor for corrupt-bitrate sources (see masterAverageBandwidth above).
    var masterBandwidth: Int? {
        let avg = masterAverageBandwidth ?? 100_000_000
