...
final class HLSLocalServer: @unchecked Sendable {
    ...
    /// PiliGlass: the port of the most recent server instance. Session churn
    /// (quality/P switching, reopen) used to rebind a fresh ephemeral port
    /// every time, which changed AVPlayer's origin URL and showed up as
    /// repeated "Listening on port X" -> "port Y" rebinds in logs. New
    ...
    func start() throws {
        ...
        // PiliGlass: reuse the last assigned port when possible (see
        // lastAssignedPort above); kernel-assigned ephemeral otherwise.
        ...
        // swapped to the LAN IP while external playback is active. Serves the current stream only.
        addr.sin_addr.s_addr = inet_addr("0.0.0.0")

        let candidates: [UInt16] = [HLSLocalServer.lastAssignedPort, 0]
        var bound = false
        var boundPort: UInt16 = 0
        for candidate in candidates {
            addr.sin_port = candidate == 0 ? 0 : candidate.bigEndian
            let attempt = withUnsafePointer(to: &addr) { ptr -> Int32 in
        ...
        stateLock.unlock()
        if assignedPort > 0 {
            HLSLocalServer.lastAssignedPort = assignedPort
        }
