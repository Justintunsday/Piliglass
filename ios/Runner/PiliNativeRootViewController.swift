...
private final class PiliNativeViewModel: ObservableObject {
  ...
  func retryComments() {
    guard let oid = commentOID else {
      refreshCurrentVideoDetail()
      return
    }
    loadComments(oid: oid, type: commentType)
  }
    ...
    NavigationStack {
      SwiftUI.Group {
        if model.homeLoading && model.homeVideos.isEmpty {
          ...
          PiliNativeErrorView(message: "暂无推荐内容") { model.refresh("home") }
        } else {
          ScrollView {
            LazyVStack(alignment: .leading, spacing: PiliNativeDesign.spaceL) {
              ...
              if model.homeVideos.count > 1 {
                HStack(alignment: .firstTextBaseline, spacing: PiliNativeDesign.spaceS) {
                  Text("继续发现")
                    .font(PiliNativeDesign.heading)
                  Rectangle()
                    .fill(PiliNativeDesign.divider)
                    .frame(height: 1)
...
private struct PiliNativeDynamicsView: View {
  ...
  var body: some View {
    NavigationStack {
      Group {
        if !model.account.isLogin {
          ...
          PiliNativeEmptyView(
            ...
            title: "还没有关注动态",
            subtitle: "关注喜欢的创作者后，更新会显示在这里"
          )
        } else {
          ScrollView {
            LazyVStack(alignment: .leading, spacing: PiliNativeDesign.spaceM) {
              HStack(alignment: .firstTextBaseline) {
                Text("关注更新")
                  .font(PiliNativeDesign.heading)
                Spacer()
                Text(piliLocalizedFormat("%d 条动态", model.dynamics.count))
                  .font(PiliNativeDesign.caption.monospacedDigit())
                  .foregroundStyle(.secondary)
              }
              ...
              if model.dynamicsLoadingMore {
                ProgressView().frame(maxWidth: .infinity).padding(.vertical, PiliNativeDesign.spaceL)
              }
            }
      ...
      .navigationBarItems(
        trailing: Button(action: { model.isSearchPresented = true }) {
          Image(systemName: "magnifyingglass")
...
private struct PiliNativeDynamicRow: View {
  ...
  var body: some View {
    HStack(alignment: .top, spacing: PiliNativeDesign.spaceS) {
      Button(action: action) {
        PiliRemoteImage(urlString: item.avatar)
          .frame(width: 42, height: 42)
          .clipShape(Circle())
          .overlay(Circle().stroke(PiliNativeDesign.divider, lineWidth: 1))
          .frame(width: PiliNativeDesign.touchTarget, height: PiliNativeDesign.touchTarget)
      }.buttonStyle(PlainButtonStyle())
      ...
      VStack(alignment: .leading, spacing: PiliNativeDesign.spaceS) {
        Button(action: action) {
          VStack(alignment: .leading, spacing: PiliNativeDesign.spaceS) {
            ...
            if !item.title.isEmpty {
              Text(item.title)
                .font(PiliNativeDesign.body.weight(.semibold))
                .foregroundColor(.primary)
                .lineLimit(2)
            }
        ...
        Button(action: action) {
          HStack(spacing: PiliNativeDesign.spaceL) {
...
private struct PiliNativeMineView: View {
  ...
  @ViewBuilder private var favoritesContent: some View {
    if !model.account.isLogin {
      ...
      ScrollView(.horizontal, showsIndicators: false) {
        LazyHStack(alignment: .top, spacing: PiliNativeDesign.spaceM) {
  ...
  private func toolbarButton(_ title: String, _ icon: String, action: @escaping () -> Void) -> some View {
    ...
    .accessibilityLabel(piliLocalized(title))
  }
...
private struct PiliNativeAccountStat: View {
  ...
  var body: some View {
    Button(action: action) {
      VStack(spacing: PiliNativeDesign.spaceXS) {
        Text(String(value))
          .font(PiliNativeDesign.heading.monospacedDigit())
          .foregroundStyle(.primary)
...
private enum PiliNativeVideoDetailTab: String, CaseIterable, Identifiable, Hashable {
...
private struct PiliNativeVideoDetailView: View {
  ...
  private func nativeTitleAndStats(_ video: PiliNativeVideoDetail) -> some View {
    VStack(alignment: .leading, spacing: 9) {
      DisclosureGroup(isExpanded: $descriptionExpanded) {
        ...
        VStack(alignment: .leading, spacing: 5) {
          Text(video.title)
            ...
            .fixedSize(horizontal: false, vertical: true)
            .multilineTextAlignment(.leading)
          if !descriptionExpanded && !video.description.isEmpty {
      ...
      HStack(spacing: 12) {
        if !video.viewText.isEmpty { Label(video.viewText, systemImage: "eye") }
        if !video.danmakuText.isEmpty { Label(video.danmakuText, systemImage: "text.bubble") }
        if !video.pubdateText.isEmpty { Label(video.pubdateText, systemImage: "calendar") }
      }
      .font(.footnote)
      .foregroundStyle(.secondary)
    }
  ...
  private func nativeActionRow(_ video: PiliNativeVideoDetail) -> some View {
    VStack(spacing: 10) {
      HStack(spacing: 0) {
        Button { model.performVideoAction("like", video: video) } label: {
        ...
        Button { model.performVideoAction("coin", video: video) } label: {
          PiliNativeVideoMetric(
        ...
        Button { model.performVideoAction("favorite", video: video) } label: {
          PiliNativeVideoMetric(
        ...
        Button { selectedTab = .comments } label: {
      ...
      if model.videoActionLoading { ProgressView().controlSize(.small) }
    }
    .padding(.vertical, 5)
  }

  private func nativePartsSection(_ video: PiliNativeVideoDetail) -> some View {
    Picker("当前视频", selection: Binding(
      get: { selectedPart },
      set: { index in
        guard let part = video.pages.first(where: { $0.index == index }) else { return }
        selectedPart = part.index
        model.selectOriginalPlayerPart(part)
      }
    )) {
      ForEach(video.pages) { part in
        Text("P\(part.index) · \(part.title)").tag(part.index)
      }
    }
    .pickerStyle(.menu)
    .accessibilityLabel("选择视频分集")
  }
        ...
        HStack(spacing: 12) {
          ...
          VStack(alignment: .leading, spacing: 2) {
            ...
            if !member.title.isEmpty {
              Text(member.title).font(.footnote).foregroundStyle(.secondary)
            }
          }
          Spacer()
  ...
  private var nativeCommentComposerBar: some View {
    HStack(spacing: 12) {
      ...
      .tint(.secondary)
      Button { model.beginDynamicComment() } label: {
      ...
      Form {
        Section {
          TextEditor(text: $model.dynamicComposerText)
            .font(.body)
            .frame(minHeight: 180)
            .accessibilityLabel(model.dynamicComposerHint)
        } header: {
        ...
        Section {
          HStack {
            Text("字数")
            Spacer()
            if model.dynamicComposerText.count > 1000 {
              Text("\(model.dynamicComposerText.count)/1000").foregroundStyle(.red)
            } else {
              Text("\(model.dynamicComposerText.count)/1000").foregroundStyle(.secondary)
            }
          }
    ...
    NavigationStack {
      if let root = model.commentThreadRoot {
        ...
        .toolbar {
          ...
          ToolbarItem(placement: .confirmationAction) {
            Button("回复") { model.beginCommentReply(root) }
          }
        }
      } else {
        ...
        HStack(spacing: 7) {
          ...
          Button(action: toggleLike) {
            Label(comment.like > 0 ? String(comment.like) : "", systemImage: comment.liked ? "hand.thumbsup.fill" : "hand.thumbsup")
              .font(.caption)
              .foregroundStyle(comment.liked ? piliAccent : .secondary)
          }
          .buttonStyle(.borderless)
        }
